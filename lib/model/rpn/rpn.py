...
class _RPN(nn.Module):
    ...
    def __init__(self, din):
        ...
        # what are the scales and ratios?
        ...
        # How come this is a conv2d layer?
        self.RPN_bbox_pred = nn.Conv2d(512, self.nc_bbox_out, 1, 1, 0)


        # define proposal layer -> refining the estimates
    ...
    def forward(self, base_feat: torch.Tensor, im_info,
                gt_boxes: torch.Tensor, num_boxes: torch.Tensor):
        """
        Paramters
        ---------
        base_feat: torch.Tensor
            The feature map from the initial convolution of the backbone
        ...
        # The probability that at at each feature map point, ther is one of 18
        # boxes
        ...
        rois = self.RPN_proposal((rpn_cls_prob.data, rpn_bbox_pred.data,
                                  im_info, cfg_key))
        # the RoI, 5 values think 0 is batchid and 1:6 is location
        ...
        if self.training:
            ...
            # score and label have the same dim

            rpn_keep = Variable(rpn_label.view(-1).ne(-1).nonzero().view(-1))
            # sampling 256, don't know how
